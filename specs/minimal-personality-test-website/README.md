# Minimal Personality Test Website Documentation

## Overview
This is a minimal, static personality test website built with React and Vite. It is designed for mobile-first use, with no data persistence or tracking.

## Features
- Take a simple personality test
- View result and detailed explanation
- See test purpose and privacy info
Minimal design, fast load
100% client-side, no backend

## Accessibility
- ARIA roles and labels for all interactive elements
- Keyboard navigation supported
- Focus indicators on headings and buttons

## Privacy
- No cookies, tracking, or data collection
- All logic runs locally in your browser

## Usage
1. Open the site in your browser
2. Click "Start Test" to begin
3. Answer each question
4. View your result and details
5. Optionally, view test purpose and privacy info

## Manual Testing Checklist
- [x] All navigation works via keyboard
- [x] ARIA labels present on all buttons
- [x] Result and details display correctly
- [x] Privacy info is accessible
- [x] No errors in console

## Source Structure
- `src/components/`: All React components
- `src/App.jsx`: Main app logic
- `public/`: Static assets

## Performance
Uses Vite for fast static builds
Minimal bundle size
No external dependencies except React

---
For more details, see the source code and comments.
